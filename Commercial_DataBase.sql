...
SELECT 
    ...
    TITLE,
    SALARY,
    LEFT(FIRST_NAME + REPLICATE(' ', 20), 20) AS FORMATTED_FIRST_NAME,
    LEFT(LAST_NAME + REPLICATE(' ', 30), 30) AS FORMATTED_LAST_NAME,
...
-- 4. Display products that meet the following criteria: 
...
-- 5. Display customers who reside in the same region as supplier 1, meaning they share the same country, city, and the last three digits of the postal code. 
...
SELECT 
	...
	S.ADDRESS, 
	S.PHONE, 
	P.CATEGORY_CODE AS BEVERAGE_PRODUCT_CODE
FROM PRODUCTS P
JOIN SUPPLIERS S
	ON P.SUPPLIER_NUMBER = S.SUPPLIER_NUMBER
WHERE 
	P.CATEGORY_CODE = 1
ORDER BY P.SUPPLIER_NUMBER;
...
LEFT JOIN CATEGORIES CS
	ON P.CATEGORY_CODE = CS.CATEGORY_CODE
WHERE C.CITY = 'Berlin'
GROUP BY C.CUSTOMER_CODE
	HAVING SUM(CASE WHEN CS.CATEGORY_NAME = 'Desserts' THEN 1 ELSE 0 END) <=1;


--9. Display customers who reside in France and the total amount of orders they placed every Monday in April 1998 (considering customers who haven't placed any orders yet). 
...
LEFT JOIN ORDERS O
    ON C.CUSTOMER_CODE = O.CUSTOMER_CODE
WHERE C.COUNTRY = 'FRANCE' AND (O.ORDER_DATE IS NULL
 OR (O.ORDER_DATE BETWEEN '1998-04-01' AND '1998-04-30' 
...
GROUP BY 
    ...
    C.PHONE
HAVING 
    COUNT(DISTINCT P.PRODUCT_REF) = (SELECT COUNT(PRODUCT_REF) FROM PRODUCTS);
...
SELECT 
    C.CUSTOMER_CODE, 
    COUNT(OD.QUANTITY) AS NO_OF_ORDERS
FROM CUSTOMERS C
LEFT JOIN ORDERS O
	ON C.CUSTOMER_CODE = O.CUSTOMER_CODE
LEFT JOIN ORDER_DETAILS OD 
...
SELECT
    ...
    ABS((SELECT COUNT(*) FROM ORDERS WHERE YEAR(ORDER_DATE) = 1996) -
